...
export function validateNseTargetName(targetName: string): void {
...
export function validateNseBundleSuffix(bundleSuffix: string): void {
...
export function deriveNseBundleIdentifier(
...
export function renderNotificationServiceSwift(): string {
  return NOTIFICATION_SERVICE_SWIFT;
}

export function renderNseInfoPlist(options: { targetName: string }): string {
  return NSE_INFO_PLIST_TEMPLATE.replace(/\{\{TARGET_NAME\}\}/g, options.targetName);
}

export function renderNseEntitlementsPlist(): string {
