...
const { normalizeAndroidNotificationIcons } = require('../options');

const ANDROID_NOTIFICATION_ICON_DENSITIES = [
...
function withNotifyKitAndroidNotificationIcons(config, icons) {
  if (icons.length === 0) {
    return config;
  }
...
async function generateNotifyKitAndroidNotificationIconResources(
  projectRoot,
  platformProjectRoot,
  icons,
) {
...
function renderNotifyKitAndroidNotificationIconsKeepXml(icons) {
...
function resolveAndroidNotificationIconSource(projectRoot, icon) {
...
function validateAndroidNotificationIconPngSignature(icon, sourcePath) {
  let sourceHeader;
  try {
...
function resolveAndroidPlatformProjectRoot(platformProjectRoot) {
  const resolvedPlatformProjectRoot = path.resolve(platformProjectRoot);
  let platformProjectRootStats;
  let canonicalPlatformProjectRoot;
...
function getAndroidNotificationIconOutputPaths(resourceRoot, preparedIcons) {
...
function lstatAndroidOutputPathIfExists(outputPath) {
  try {
...
function validateAndroidResourceCollisions(platformProjectRoot, resourceRoot, preparedIcons) {
...
function findResourceDirectories(sourceSetRoot, resourceType) {
...
function findLogicalResourceCollision(resourceDirectory, resourceName) {
...
function validateDrawableDestination(drawableDirectory, resourceName) {
...
function validateKeepFileDestination(resourceRoot) {
...
async function generateAllAndroidNotificationIconBuffers(projectRoot, resourceRoot, preparedIcons) {
...
function hasLogicalResourceName(fileName, resourceName) {
