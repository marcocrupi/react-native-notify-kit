...
- **FCM Mode / iOS**: validated foreground delivery on a physical iPhone with real FCM payloads and a generated NSE, including logical iOS attachment metadata in `getDisplayedNotifications()`. Visible FCM background tap was also validated with a NotifyKit background press event; the smoke marker correlated the tap through the notification id. Killed state, visual lock-screen/banner attachment rendering, textual NSE logs, and data-only RNFirebase client-handler paths are not claimed as verified in this release.
