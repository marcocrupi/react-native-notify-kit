...
- **iOS Expo validation scope.** The 10.4.0 Expo gate verified runtime base behavior, foreground FCM delivery on a physical iPhone, logical `ios.attachments` metadata, the `NotifyKitNSE` process, and visible FCM background tap with a NotifyKit background press event. The smoke marker correlated the tap through the notification id. It did not verify iOS killed state, visual lock-screen/banner attachment rendering, textual NSE log capture, or iOS RNFirebase data-only/client-handler behavior.
...
- **RNFirebase modular API warning cleanup remains a follow-up.** The Expo fixture may still surface non-fatal RNFirebase modular API warnings until that cleanup is addressed.
