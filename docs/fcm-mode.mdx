...
- The **Expo config plugin** generates and wires the iOS NSE during Expo prebuild. For bare React Native, the **CLI** (`npx react-native-notify-kit init-nse`) scaffolds the same NSE target, patches the Podfile, and wires the `.pbxproj`.
...
- You need Expo Go. FCM Mode on iOS requires a native Notification Service Extension, so use Expo CNG / prebuild development builds instead.
...
Expo CNG support is available for FCM Mode on iOS. The config plugin can generate and wire the iOS Notification Service Extension during prebuild, so Expo development build apps can use NotifyKit FCM Mode without manually creating the NSE in Xcode.

Expo Go is not supported. This path requires native targets, an embedded `.appex`, CocoaPods, signing, and a development build or EAS build.

### Expo config

Boolean `true` enables the default NSE target:
...
Android Expo base runtime has been validated with prebuild and a development build. Android config plugin automation for foreground service declarations, exact-alarm permissions, full-screen intent, and related advanced Android setup is not part of 10.4.0.
...
- **Expo Go not supported.** Expo CNG / prebuild development builds are supported for iOS FCM Mode via the config plugin. Expo Go cannot load the required native target or embedded NSE.
- **Android Expo advanced automation is future work.** Android Expo base runtime has been validated, but 10.4.0 does not add Android config plugin automation for foreground services, exact alarms, full-screen intent, or related advanced permissions.
- **10.4.0 validation scope.** The Expo release gate covered real tarball install, prebuild, `pod install`, `xcodebuild`, `.appex` generation/embedding, Expo iOS/Android runtime smoke, and foreground iOS FCM Mode delivery on a physical iPhone with logical attachment metadata. It did not complete tap, killed state, visual lock-screen/banner attachment rendering, textual NSE log capture, or data-only RNFirebase client-handler validation.
