...
Repeating triggers (`HOURLY` / `DAILY` / `WEEKLY` / `MONTHLY`) honor wall-clock time across DST transitions on Android by using calendar-aware scheduling.

Android clears `AlarmManager` registrations when the device reboots, so the OS alarm entry is not enough on its own. NotifyKit persists trigger notifications and re-arms them after a normal reboot through its `RebootBroadcastReceiver`. Future one-shot `TriggerType.TIMESTAMP` triggers are part of this recovery path; you do not need to make a trigger recurring or daily only to survive reboot, and push notifications are not required for this local scheduling path.

Since v9.6.0 the library also detects boot deltas via `Settings.Global.BOOT_COUNT` and reschedules persisted alarms on app cold start. This protects OEM devices that delay or suppress `BOOT_COMPLETED` (Xiaomi MIUI, Oppo ColorOS, Huawei EMUI, Vivo FuntouchOS). If an OEM suppresses the boot broadcast, opening the app after reboot can exercise this cold-start recovery path, but delivery is still subject to vendor autostart and background restrictions. Stale non-repeating triggers (timestamp already in the past at recovery time) fire once within a 24-hour grace period and are then deleted; beyond the grace period they're deleted silently.

Reboot recovery and exact fire timing are separate concerns. Recovery means the persisted trigger is re-armed after reboot. Exact timing depends on the Android alarm primitive and permission state described below.
...
Starting from Android 12, the exact `AlarmManager` APIs used by `AlarmType.SET_EXACT`, `AlarmType.SET_EXACT_AND_ALLOW_WHILE_IDLE`, and `AlarmType.SET_ALARM_CLOCK` require exact-alarm access through [`SCHEDULE_EXACT_ALARM`](https://developer.android.com/reference/android/Manifest.permission#SCHEDULE_EXACT_ALARM) or `USE_EXACT_ALARM`. If exact-alarm access is not granted, NotifyKit still persists the timestamp trigger and falls back to an inexact `setAndAllowWhileIdle` alarm instead of failing the schedule request. The trigger is not lost because of this fallback, but exact fire timing is not guaranteed without permission.

If your app needs exact fire timing, check the alarm setting before scheduling exact alarms, explain why the permission matters, and send the user to the system Alarms & Reminders screen when needed:
...
When the user changes exact-alarm permission, Android notifies the app and NotifyKit reschedules persisted triggers with the best available alarm primitive. If permission remains denied, recovered triggers continue on the inexact fallback path. For alarm-clock or timer-like use cases where exact visible alarm behavior is core to the app, consider `AlarmType.SET_ALARM_CLOCK` within Android and Google Play policy constraints.
