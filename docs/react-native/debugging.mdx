...
- **Host target build cycle after adding the NSE** — the CLI automatically strips RNFB-style `INFOPLIST_FILE` input paths from the host target to prevent this, but if incremental builds still hang look for other `INFOPLIST_FILE` references in the host target's build settings.
