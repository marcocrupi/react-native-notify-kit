...
> This page covers the visual timestamp and chronometer (stopwatch / countdown) fields shown on an already-displayed notification. To **schedule** a notification to fire at a specific time or on a recurring interval, see [Trigger Notifications](/react-native/triggers) and the `AlarmType` reliability guide.
