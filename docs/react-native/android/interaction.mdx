...
"Clear all" button on the device or by physically swiping them away. Notify Kit allows the user to interact
with notifications, and also hook onto events when the interaction occurs.

## Press Action

By default, tapping a notification opens the app's main activity — equivalent to `pressAction: { id: 'default', launchActivity: 'default' }`. This default is applied when `android.pressAction` is omitted. To create a non-tappable notification, pass `pressAction: null` explicitly. In that opt-out state, the notification body does not open the app, emit `PRESS`, or populate `getInitialNotification()`. The Android guidelines suggest a notification should always be tappable, but opt-outs are sometimes useful for status-only displays.

You can override the default to customize exactly _how_ the application opens, to launch a different Activity, or to run a background task — the options are described below.

If the notification body is pressed and body tap is enabled, a `PRESS` [event](/react-native/events) is published to subscribers. Action buttons are independent from the body tap and emit `ACTION_PRESS` separately.
...
Notify Kit will now attempt to launch the application when the user presses the notification. View the
...
Each action in a notification's `android.actions` array must use a unique `pressAction.id` within that array. The same
ID may be reused by actions in different notifications, and the notification body's `android.pressAction.id` may match
an action ID. Use action IDs with [events](/react-native/events) to hook into what action was pressed. When pressed, an
action sends the `ACTION_PRESS` event type to any subscribers:
...
`pressAction` can be used. Notify Kit will use this activity to launch your application:
