...
> **Foreground service notifications**: when `asForegroundService: true`, `ongoing` defaults to `true` even if you don't set it — this prevents the user from dismissing the notification on Android 13, matching pre-Android 13 platform behavior. On Android 14+, dismissed foreground service notifications are automatically re-posted while the service is active (Android 14 ignores `FLAG_ONGOING_EVENT` for most service types). Set `ongoing: false` explicitly if you want the user-dismissible behavior — not recommended for most FGS use cases.
...
Running this code, Notify Kit will attempt to launch the activity when the notification is displayed.
...
To check whether the app currently has Full-Screen Intent access:
...
`AndroidNotificationSetting.ENABLED` indicates that Full-Screen Intent access is available; it does not guarantee that
a particular notification will be presented full-screen. Android determines actual presentation based on the
notification configuration, device state, and other platform conditions.
