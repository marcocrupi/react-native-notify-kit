...
On iOS, `EventType.DELIVERED` is emitted to `onForegroundEvent` whenever a Notifee-owned notification is presented to the user while the app is in foreground. This includes local `displayNotification` calls, trigger notifications that fire in foreground, and `handleFcmMessage` foreground displays. See the [behaviour guide](/react-native/ios/behaviour#foreground-delivery-events) for the iOS-specific caveats (background / killed trigger notifications do not emit DELIVERED on iOS).
