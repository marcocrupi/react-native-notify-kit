...
- Validated iOS FCM Mode foreground delivery on a physical iPhone with a generated NSE and logical attachment metadata visible through `getDisplayedNotifications()`.
- Validated iOS visible FCM background tap with a NotifyKit background press event. The smoke marker correlated the tap through the notification id.
...
- iOS tap validation covers visible FCM background notification tap only. iOS killed state, visual lock-screen/banner attachment rendering, textual NSE log capture, and iOS RNFirebase data-only/client-handler validation were not completed as release gates for 10.4.0.
...
- RNFirebase modular API warning cleanup remains a follow-up.
