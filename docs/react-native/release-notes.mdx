...
- Android tap reopened the app during executor validation, but `SMOKE:BACKGROUND_EVENT_PRESS` was not observed and remains a follow-up.
- iOS tap, iOS killed state, visual lock-screen/banner attachment rendering, textual NSE log capture, and iOS RNFirebase data-only/client-handler validation were not completed as release gates for 10.4.0.
