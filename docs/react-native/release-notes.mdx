...
- **Custom timestamp repeat intervals**: `TimestampTrigger.repeatInterval` supports calendar-based custom intervals such as every 2 days, every 2 weeks, and every 3 months from the selected start timestamp. `RepeatFrequency.MONTHLY` adds monthly timestamp recurrence; Android advances triggers with calendar-aware scheduling, while iOS uses a bounded rolling schedule of one-shot notifications that is topped up when the app runs or the user interacts with notifications.

### Changed

- **iOS compatibility cleanup**: deprecated notification summary and announcement fields are now compatibility no-ops on supported iOS versions, and `foregroundPresentationOptions.alert` uses modern banner/list presentation options instead of the deprecated native alert option.
- **iOS call intents and launch handling**: use `IOSIntentIdentifier.START_CALL` for call-related category intent identifiers. Legacy `START_AUDIO_CALL` and `START_VIDEO_CALL` values remain accepted for compatibility, but map to Apple's unified call intent identifier and `getNotificationCategories()` may return `START_CALL`. The legacy `UILocalNotification` launch-options fallback was removed; `getInitialNotification()` remains supported through the modern `UNNotificationResponse` path.
- **Android/build hardening**: small owned Android warning sources were cleaned up, including Gradle DSL assignment syntax, deprecated React Native activity access in the Kotlin bridge, `RebootBroadcastReceiver` action filtering, `Locale.ROOT` resource-name normalization, and removal of the invalid smoke-app `FOREGROUND_SERVICE_SHORT_SERVICE` permission.
- **Tooling**: reference generation is now idempotent because generated docs are formatted with Prettier, and the smoke app has a lightweight Jest import/render gate.
