...
Release focused on the first official Expo CNG integration for FCM Mode on iOS. This is support for Expo prebuild and development builds, not Expo Go.

### Expo CNG

- Added an official config plugin entry for `react-native-notify-kit`.
- The plugin can generate and wire the `NotifyKitNSE` Notification Service Extension during Expo prebuild, so Expo development builds can use the iOS FCM Mode attachment/rich-notification path without manually creating the NSE in Xcode.
- The plugin supports `ios.notificationServiceExtension: true` and an object form with `enabled`, `targetName`, and `bundleSuffix`.
- Expo Go is not supported because the feature requires native iOS targets.
...
- Validated iOS FCM Mode foreground delivery on a physical iPhone with a real generated NSE and logical attachment metadata visible through `getDisplayedNotifications()`.
- Tap handling, killed-state delivery, visual lock-screen/banner attachment rendering, textual NSE logs, and data-only RNFirebase client-handler paths were not completed as release gates for 10.4.0.
