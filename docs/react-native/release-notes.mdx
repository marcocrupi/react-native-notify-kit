...
This 10.7.2 maintenance update fixes stale state when long-lived Android foreground-service notifications are updated repeatedly with the same ID.

### Android Foreground Services

- Fixed same-ID updates retaining stale first-notification state, which could leave later re-post and timeout paths using obsolete content. The current foreground-service state now follows the latest update submitted to Android by a platform call that returned normally. [Issue #75](https://github.com/marcocrupi/react-native-notify-kit/issues/75).

## [10.7.1] (2026-09-07)

This 10.7.1 maintenance update fixes Android notification timestamp preservation in FCM Mode and the Server SDK.

### FCM Mode / Server SDK

- Fixed Android `showTimestamp` and `timestamp` handling so `buildNotifyKitPayload()` preserves both fields in `_v: 1` `notifee_options` through client reconstruction, enabling the existing Android timestamp display and sorting behavior for FCM Mode notifications. Issue #73.

## [10.7.0] (2026-08-31)

This 10.7.0 update adds Expo CNG support for generating Android small notification icon resources from project-relative PNG files.

### Expo Android Notification Icons

- Added `android.icons` config-plugin support for generating one or more density-specific Android small notification icon resources during CNG/prebuild.
- Each configured resource name can be used with `notification.android.smallIcon`.
- The plugin writes an Android resource-shrinker keep rule for generated icons. Only `type: 'small'` is supported.
