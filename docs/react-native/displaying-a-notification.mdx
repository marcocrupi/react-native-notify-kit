...
description: Display your first cross-platform notification with Notify Kit.
---
  ...
  async function onDisplayNotification() {
    ...
    await notifee.displayNotification({
      ...
      android: {
        ...
        // Tapping the notification opens the app by default (since v9.3.0,
        // pressAction defaults to { id: 'default', launchActivity: 'default' }).
        // Set pressAction: null explicitly to create a non-tappable notification.
      },
    });
  }
...
Requesting permission is required on iOS and on Android 13+ (`POST_NOTIFICATIONS`). You need to ask the user to enable notifications by calling `requestPermission` before displaying any notifications.

> To learn more about requesting permissions, view the [iOS Permissions](/react-native/ios/permissions) and [Android Permissions](/react-native/android/permissions) documentation.
...
In addition to channels, small icons are an Android-only concept and are required to display a notification. If no icon is specified, Notify Kit will fallback to the default launcher icon (`ic_launcher`).
...
async function onDisplayNotification() {
  ...
  // Required for iOS and Android 13+ — see the permissions docs
  await notifee.requestPermission();
...
On Android 13+, if `POST_NOTIFICATIONS` is not granted, posting the notification can fail safely instead of displaying anything. When Android rejects the post before `notify()` is called, Notify Kit does not emit a `DELIVERED` event for that notification.
